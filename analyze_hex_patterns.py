import itertools
from collections import Counter

# List of hex strings
hex_strings = [
    "160111433b00035f536110435a380402561240555c526e1c0e431300091e4f04451d1d490d1c49010d000a0a4510111100000d434202081f0755034f13031600030d0204040e",
    "",
    "",
    "0b07540c1d0d0b4800354f501d131309594150010011481a1b5f11090c0845124516121d0e0c411c030c45150a16541c0a0b0d43540c411b0956124f0609075513051816590026004c061c014502410d024506150545541c450110521a111758001d0607450d11091d00121d4f0541190b45491e02171a0d49020a534f",
    "",
    "",
    "011b0d131b060d4f5233451e161b001f59411c090a0548104f431f0b48115505111d17000e02000a1e430d0d0b04115e4f190017480c14074855040a071f4448001a050110001b014c1a07024e5014094d0a1c541052110e54074541100601014e101a5c",
    "0c06004316061b48002a4509065e45221654501c0a075f540c42190b165c"
]

# Convert hex strings to byte arrays
byte_arrays = [bytes.fromhex(hex_str) for hex_str in hex_strings]

# Calculate the frequency distribution of each byte from the original hex strings
original_bytes = b''.join(byte_arrays)
original_frequency_distribution = Counter(original_bytes)

# Function to XOR two byte arrays and return the result as a byte array
def xor_bytes(b1, b2):
    return bytes(a ^ b for a, b in zip(b1, b2))

# Find the XOR results of all possible pairs
xor_results = []
for b1, b2 in itertools.combinations(byte_arrays, 2):
    if len(b1) > len(b2):
        b2 = b2.ljust(len(b1), b'\x00')
    elif len(b2) > len(b1):
        b1 = b1.ljust(len(b2), b'\x00')
    xor_results.append(xor_bytes(b1, b2))

# Flatten the list of XOR results into a single list of bytes
all_xor_bytes = b''.join(xor_results)
# Calculate the frequency distribution of each byte from the XOR results
xor_frequency_distribution = Counter(all_xor_bytes)

# Sort the frequency distribution from most frequent to least frequent
sorted_original_frequency = sorted(original_frequency_distribution.items(), key=lambda x: x[1], reverse=True)
sorted_xor_frequency = sorted(xor_frequency_distribution.items(), key=lambda x: x[1], reverse=True)

# Print the sorted frequency distribution for original hex strings
print("Frequency distribution of bytes in original hex strings:")
for byte, count in sorted_original_frequency:
    print(f"Byte {byte:02x}: {count}")

# Print the sorted frequency distribution for XOR results
print("\nFrequency distribution of bytes in XOR results:")
for byte, count in sorted_xor_frequency:
    print(f"Byte {byte:02x}: {count}")
